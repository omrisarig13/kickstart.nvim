...
vim.keymap.set('n', '<leader>pft', format_table, { desc = '[P]ersonal [F]ormat [T]able' })
...
vim.keymap.set('n', '<leader>pcd', ':Lcdc<cr>', { desc = '[P]ersonal [C]hange [D]irectory' })
...
require('lazy').setup({
  ...
  { -- Collection of various small independent plugins/modules {{{
    ...
    config = function()
      ...
      -- Add pairs when writing opening pairs.
      -- Disable if gets annoying.
      ...
      vim.keymap.set('n', '<leader>ifo', MiniFiles.open, { desc = 'M[I]ni [F]ile explorer [O]pen' })
      vim.keymap.set('n', '<leader>ifc', function()
        MiniFiles.open(vim.api.nvim_buf_get_name(0))
      end, { desc = 'M[I]ni [F]ile explorer open [C]urrent file' })
      vim.keymap.set('n', '<leader>ifi', function()
        local file_path = vim.fn.input('Enter path to open:', '', 'file')
        MiniFiles.open(file_path)
      end, { desc = 'M[I]ni [F]ile explorer open [I]nputted path' })
