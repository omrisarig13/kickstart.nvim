...
local function open_merge()
  ...
  -- Open master version
  local master_file = '../master/' .. clipboard
  ...
  vim.cmd('vsplit ../gen-9/' .. vim.fn.fnameescape(clipboard))
...
vim.keymap.set('n', '<leader>pe', open_merge, { desc = '[P]ersonal open m[E]rge' })
...
-- Mouse toggling {{{
-- Create a function that checks the mouse status (whether it is 'a' or '') and
-- toggle it to the other value.
